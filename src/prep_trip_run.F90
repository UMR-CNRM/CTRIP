...
USE MODN_TRIP, ONLY : CGROUNDW, CVIT, LFLOOD,  &
                      ...
                      LGWSUBF, XGWSUBD, LCALCRIVLEN, LCALCNEARAQ, &
                      CLAKE, LBACKWATER, XBANKSLOPE
USE MODN_TRIP_ASSIM, ONLY : LASSIM, CLOCAL_FILE, CINFL, XINFL_INIT
!
...
!
REAL,DIMENSION(:,:),    ALLOCATABLE  :: ZLOCAL_LEN
!
...
IF(CGROUNDW=='DIF'.AND.LCPL_LAND.AND.LCPL_GW)THEN
  ALLOCATE(TP%XNEAR_AQUI (ILON,ILAT))
ELSE
...
!
IF(LASSIM)THEN
  ALLOCATE(ZLOCAL_LEN(ILON,ILAT))
ELSE
  ALLOCATE(ZLOCAL_LEN(0,0))
ENDIF
!
!-------------------------------------------------------------------------------
...
!
  IF(LCPL_LAND.AND.LCPL_GW)THEN
    YVAR ='NEAR_AQUI'
    IF(LCALCNEARAQ)THEN
      CALL TRIP_NEAR_AQUI(TP,TPG,ILON,ILAT)
    ELSE
      CALL READ_TRIP(NLISTING,YFILE_READ,YVAR,ZREAD)
      TP%XNEAR_AQUI(:,:) = ZREAD(ILON_DEB:ILON_END,ILAT_DEB:ILAT_END)
    ENDIF
    CALL WRITE_TRIP(NLISTING,YFILE_PARAM,YVAR,TPG%GMASK,TP%XNEAR_AQUI)
  ENDIF
!
  YVAR ='WEFF'
  CALL READ_TRIP(NLISTING,YFILE_READ,YVAR,ZREAD)
...
!
  YVAR='LOCAL_LEN'
  CALL READ_TRIP(NLISTING,CLOCAL_FILE,YVAR,ZREAD)
  ZLOCAL_LEN(:,:) = ZREAD(ILON_DEB:ILON_END,ILAT_DEB:ILAT_END)
  CALL WRITE_TRIP(NLISTING,YFILE_PARAM,YVAR,TPG%GMASK,ZLOCAL_LEN)
!
  IF(CINFL=='A09'.OR.CINFL=='S21')THEN
    ALLOCATE(ZWORK(ILON,ILAT))
    ZWORK(:,:) = XINFL_INIT
    YVAR ='INFL'
    CALL WRITE_TRIP(NLISTING,YFILE_PREP,YVAR,TPG%GMASK,ZWORK,ODOUBLE=LDOUBLE)
    DEALLOCATE(ZWORK)
  ENDIF
!
ENDIF
