...
!
    ...
    ENDIF
    WHERE(TPST%GMASK(:)) TPST%XCPL_FFLOOD(:) = 0.0
!
    ...
    ENDIF
    WHERE(TPST%GMASK(:)) TPST%XCPL_PIFLOOD(:) = 0.0
!
  ENDIF
!
ENDIF
...
!
  ...
  ENDIF
  WHERE(TPST%NGRCN(:)==9.OR.TPST%NGRCN(:)==12) TPST%XCPL_RIVDIS(:) = 0.0
!
...
!
    ...
    ENDIF
    WHERE(TPST%GMASK_GRE(:)) TPST%XCPL_CALVGRE(:) = 0.0
!
    ...
    ENDIF
    WHERE(TPST%GMASK_ANT(:)) TPST%XCPL_CALVANT(:) = 0.0
!
  ENDIF
!
ENDIF
