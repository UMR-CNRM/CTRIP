...
!
INTEGER :: JLON, JLAT, JSTATE, ISTATE, INEXTST
!
INTEGER :: IERR
...
IF (NRANK==NPIO) THEN
  ...
  DO JSTATE = 1,TPST%NNSTATE
    ...
    IF (TPG%NGRCN(JLON,JLAT)>=1.AND.TPG%NGRCN(JLON,JLAT)<=8) THEN
      ...
      INEXTST = TPST%NSTATE_IND(TPG%NNEXTX(JLON,JLAT),TPG%NNEXTY(JLON,JLAT))
      IPREVN(INEXTST) = IPREVN(INEXTST)+1
      IPREVST(IPREVN(INEXTST),INEXTST) = MOD(JSTATE-1,ISTATE)+1
    ENDIF
  ENDDO
  DEALLOCATE(IPREVN)
ENDIF
