...
!
IF(CLAKE=='MLK') THEN
  CALL NCCREATE(KLISTING,YFILE,YTITLE,YTIMEUNIT,INUM,YVNAME(1:INUM),YVLNAME(1:INUM),YUNIT(1:INUM),   &
                ZLON,ZLAT,XUNDEF,LNCPRINT,INCID,OTIME,OLAKE=LLAKE(1:INUM),KLAKE_NUM=KLAKE_NUM)
ELSE
  CALL NCCREATE(KLISTING,YFILE,YTITLE,YTIMEUNIT,INUM,YVNAME(1:INUM),YVLNAME(1:INUM),YUNIT(1:INUM),   &
