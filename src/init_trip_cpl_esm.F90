...
!
      ALLOCATE(ICODE(KLON*KLAT))
      ALLOCATE(ZNEAR(KLON*KLAT))
      ALLOCATE(ZX(KLON*KLAT))
      ALLOCATE(ZY(KLON*KLAT))
      ZNEAR(:)=XUNDEF
      ICODE(:)=-1
!
      IWORK=0
...
!
    ELSE
!
      ! NEAR_AQUI is now defined in NUM_AQUI in TRIP_PGD
      TP%XNEAR_AQUI(:,:) = TP%XNUM_AQUI(:,:)
